...
//  Naukri
//
//  Created by Himanshu Gupta on 10/01/19.
//  Copyright © 2019 Infoedge. All rights reserved.
//
...
open class NISwiftBanner: UIView {
    ...
    /// The label that displays the banner's subtitle.
    @objc public let separatorLable: UILabel = {
        let label = UILabel()
        label.numberOfLines = 0
        label.translatesAutoresizingMaskIntoConstraints = false
        return label
    ...
    @objc public let imageView: UIImageView = {
        ...
        imageView.frame = CGRect(x: 16, y: 16, width: 30, height: 40)
        return imageView
    }()
    ...
    @objc public required init(title: String? = nil,subtitle: String? = nil, backgroundColor: UIColor = UIColor.white, image: UIImage? = nil, didTapBlock: (() -> ())? = nil, separatorColor: UIColor?) {
        ...
        separatorLable.backgroundColor = separatorColor
    ...
    private func addGestureRecognizers() {
        addGestureRecognizer(UITapGestureRecognizer(target: self, action:  #selector(NISwiftBanner.didTap(_:))))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(NISwiftBanner.didSwipe(_:)))
        swipe.direction = .up
    ...
    private func initializeSubviews() {
        ...
        contentView.addSubview(separatorLable)
        contentView.addConstraint(separatorLable.constraintWithAttribute(.leading, .equal, to: contentView, constant: 0.0))
        contentView.addConstraint(separatorLable.constraintWithAttribute(.trailing, .equal, to: contentView, constant: 0.0))
        contentView.addConstraint(separatorLable.constraintWithAttribute(.bottom, .equal, to: contentView, constant: 0.0))
        separatorLable.addConstraint(separatorLable.constraintWithAttribute(.height, .equal, to: 1.5))
    ...
    private func adjustHeightOffset() {
        ...
        if superview === NISwiftBanner.topWindow() && self.position == .top {
    ...
    open func show(_ view: UIView? = nil, duration: TimeInterval? = nil) {
        let viewToUse = view ?? NISwiftBanner.topWindow()
        guard let view = viewToUse else {
            print("[NISwiftBanner]: Could not find view. Aborting.")
            return
        }
        for item in view.subviews{
            if(item .isKind(of: NISwiftBanner.self)){
